...
| `argus.telemetry.enabled` | `true` | Send anonymous usage statistics (see [Telemetry](#telemetry)) |

### Telemetry

Argus sends anonymous usage statistics to Google Analytics to help prioritise features. It is **off whenever VS Code telemetry is off** (`telemetry.telemetryLevel`), and can be disabled separately with `argus.telemetry.enabled: false`.

**What is sent:** extension / VS Code version, OS, remote type (e.g. `wsl`, `ssh-remote`), UI language, which tabs and filters are used (filter type and preset, never search text), which UI features are used (e.g. Map playback, step search, notes — once per session panel, never their content), which built-in analysis rules fire, the model family of an opened session (`opus`/`sonnet`/`haiku`), and bucketed values (e.g. `10-49`) such as number of sessions, steps, sub-agents and findings, peak context size, compaction count, cache hit ratio, transcript file size, and parse/load times. Errors are reported only as a location and error type (e.g. `refresh` / `Error`).

**What is never sent:** prompts or transcript content, file paths, project or folder names, session IDs, search text, error messages or stack traces, and the VS Code machine ID. The only identifier is a random ID generated by Argus. As with any web request, Google Analytics derives an approximate location (country/city) from the request IP; GA4 does not store the IP address itself.
